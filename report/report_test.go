...
func TestGenerateReport(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.description, func(t *testing.T) {
			var prettyReport *models.Report
			gotReport, err := cfg.GenerateReport(test.pkg)
			if err != nil {
				t.Errorf("GenerateReport failed with errors: %v", err)
			}
			err = json.Unmarshal(gotReport, &prettyReport)
			if err != nil {
				t.Errorf("unable to unmarshal report bytes into struct: %v", err)
			}
			if prettyReport != nil {
				prettyReport.ReportTime = test.wantReport.ReportTime
			}
			assert.EqualValues(t, test.wantReport, *prettyReport)
		})
	}
}
